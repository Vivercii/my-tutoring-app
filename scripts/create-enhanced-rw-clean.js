...
async function createEnhancedRWQuestions() {
  ...
  for (let i = 0; i < rwQuestions.length; i++) {
    ...
    // Extract clean answer choices
    const answerChoices = [
      extractAnswerText(csvRow['Choice A_html']) || csvRow['Choice A'] || '',
      extractAnswerText(csvRow['Choice B_html']) || csvRow['Choice B'] || '',
      extractAnswerText(csvRow['Choice C_html']) || csvRow['Choice C'] || '',
      extractAnswerText(csvRow['Choice D_html']) || csvRow['Choice D'] || ''
    ];
    
    // Create the question with proper nested create for passage
  ...
  examples.forEach((ex, i) => {
    ...
    console.log(`   Answer choices: ${ex.question.answerChoices?.slice(0, 2).join(', ')}...`);
