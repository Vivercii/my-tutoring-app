...
// Parse question to extract passage and question - FIXED VERSION
function parseQuestion(fullText, htmlText) {
    ...
    // Extended patterns for question detection
    const patterns = [
        ...
        "Which quotation",
        "Which statement",
        "Which detail, if true",  // ADD THIS
        "Which of the following",  // ADD THIS
        "What is the main",
    ...
    // IMPORTANT CHANGE: Don't use a threshold for these longer passages
    // If we found a pattern, split there regardless of position
    if (splitPoint > 0) {
        const passage = fullText.substring(0, splitPoint).trim();
        const question = fullText.substring(splitPoint).trim();
        
        // Only return if we actually got both parts
        if (passage && question && passage !== fullText) {
            return {
                passage: passage,
                question: question,
                isDualText,
...
function main() {
    ...
    const outputPath = process.argv[3] || 'sat_questions_processed_fixed.json';
    const outputDir = process.argv[4] || 'sat_questions_by_test_fixed';
    ...
    try {
        ...
        console.log('\n🎉 All processing complete with FIXED extraction!');
        console.log('You now have 100% passage extraction for all questions.');
